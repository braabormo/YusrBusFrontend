...
export default function ChangeRoleDialog({ entity, mode, onSuccess }: CommonChangeDialogProps<Role>) 
{
  ...
  const { formData, setFormData, getError, isInvalid, validate, clearError } = useEntityForm<Role>(entity, validationRules);
  ...
  return (
    <DialogContent aria-describedby={undefined} dir="rtl" className="sm:max-w-6xl max-h-[90vh] flex flex-col">
      <DialogHeader>
        ...
        {systemState.isLoading && <Loader2 className="inline-block mr-2 h-4 w-4 animate-spin text-muted-foreground" />}
      </DialogHeader>
      ...
      <DialogFooter className="gap-2">
        ...
        <SaveButton formData={formData as Role} dialogMode={mode} service={new RolesApiService()} onSuccess={(data) => onSuccess?.(data, mode)} validate={validate} />
      </DialogFooter>
    </DialogContent>
  );
}