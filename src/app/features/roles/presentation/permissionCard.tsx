...
export const categorizePermissions = (
  ...
  return order
    ...
    .map((key) => ({ resource: key, ...groups[key] }));
};
...
export function PermissionCard({
  ...
  const hasMaster = isMasterRequired 
    ? true 
    : masterPermission ? selectedPermissions.includes(masterPermission) : false;
      ...
      <CardHeader className="flex flex-row items-center justify-between border-b py-3 px-4">
        ...
        {masterPermission && !isMasterRequired && (
          <Checkbox
            checked={hasMaster}
            onCheckedChange={() => 
              onToggle(toggleGetPermission(selectedPermissions, resourceId, masterPermission))
            }
          />
        )}
      </CardHeader>
        ...
        {actions.map((action) => (
          ...
          >
            ...
            <Checkbox
              ...
              onCheckedChange={() => 
                onToggle(toggleActionPermission(selectedPermissions, action.id))
              }
            />
          </div>
        ))}
