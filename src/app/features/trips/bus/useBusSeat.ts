...
export default function useBusSeat({
  ...
  const handleSendByWhatsappTicket = async () => {
    ...
    try {
      ...
      const response = await TicketReportApiService.addToStorage(
        ticket.accessKey,
        last4,
      );

      if (response.status === 200) {
        toast.success("تم تجهيز الرابط بنجاح، جاري فتح واتساب...", {
          id: loadingToast,
        });
  ...
  const handleSendByEmailTicket = async () => {
    ...
    const loadingToast = toast.loading(
      "جاري إرسال التذكرة إلى البريد الإلكتروني...",
    );

    try {
      const last4 = ticket.passenger.phoneNumber.slice(-4);
      const response = await TicketReportApiService.sendByEmail(
        ticket.accessKey,
        last4,
      );
      ...
      toast.error("حدث خطأ غير متوقع أثناء محاولة الإرسال", {
        id: loadingToast,
      });
    }
  };
